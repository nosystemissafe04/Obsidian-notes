...
`cd HKLM:\` [[TREASURE/Powershell for Infosec/Powershell Fundamentals#Powershell Providers]]
...
NOW WITH THIS SCRIPT WE KNOW WHICH SERVICE ARE RUN BY LOCALSYSTEM AND MANUALLY STARTED AND WHICH OF THEM CAN BE STARTABLE BY US , <mark class="hltr-myblue">THE ACCESSCHK WAS JUST VERIFICATION</mark> WE ARE FINDDING THOSE SERVICES WITH SC SDSHOW COMMAND TO FIND WRITABLE SERVICE IMAGEPATH BY US WHICH IS AUTHENTICATED USER 
**IF THERE ARE NONE WE WILL NOT GET ANY OUTPUT FROM SECOND COMMAND** 

#### CHANGING THE IMAGEPATH OF THE SERVICE 
**SET-ITEMPROPERTY**

```POWERSHELL
SET-ITEMPROPERTY <SERVICE-PATH> -NAME ImagePath -value '<our injected malicious binary path or command>'
```

**BYPASSING APPLOCKER (APPLICATION WHITELISTING)**
- STORES WHICH ARE APPROVED APPS TO RUN 

[THESE ARE THE GENERIC APPLOCKER BYPASS PATHS WHICH WE CAN THROW OUR PAYLOAD IN AND EXECUTE IT ](https://github.com/api0cradle/UltimateAppLockerByPassList/blob/master/Generic-AppLockerbypasses.md)

THESE ARE SOME DIRECTORIES ARE PERMITTED BY APPLOCKER, THESE ARE ALLOWED 
...
*WHEN WE ARE WRITING TO A REGISTRY AND EDITING TS IMAGEPATH , THE IMAGEPATH WILL BE OVERWRITTEN BY OS AFTER SOME SECONDS , WE NEED TO BE FAST 
- *FIRST WE WILL EDIT IT 
- *THEN RAN THE SERVICE , WHICH WILL RUN OVER REVERSE SHELL AND WE WILL GET THE SHELL (AS WE DISCUSSED EARLIER THE PROCESS WILL DIE BCZ OS WILL CHANGE THE REGISTRY SO , WE NEED TO MIGRATE THE PROCESS OF OVER REVERSE SHELL JUST LIKE METASPLOIT DO 0
...
> [!failure] IF
> THE REGISTRY DOES NOT CHANGE , GO TO THE SERVICE REGISTRY DIRECTORY THEN CHANGE IT , IT MIGHT WORK LIKE THIS 
> 
> STARTING SERVICE LIKE THIS `START-SERVICE ` THEN TYPING THE SERVICE NAME `WUAUSERV` WORKED IN THE COURSE AND THERE IS NO EXPLANATION WHY IT HAPPENED 
