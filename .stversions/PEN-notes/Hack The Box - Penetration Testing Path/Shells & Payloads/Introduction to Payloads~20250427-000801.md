...
*This can help you to bypass AV if you can bypass AMSI* [[AMSI Bypass]] .This is something we will explore further in this module. For now, understand that the payloads we use to get a shell on a system will largely be determined by what OS, shell interpreter languages, and even programming languages are present on the target.
...
1. What command language interpreter is used to establish a system shell session with the target?
    ANS=>POWERSHELL but how come powershellis the the answer according to chatgpt [[why powershell is right answer]]
