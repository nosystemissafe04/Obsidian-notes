...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo mkdir -p /var/www/uploads/SecretUploadDirectory
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo chown -R www-data:www-data /var/www/uploads/SecretUploadDirectory
```
...
0xxNosystemisSafe@htb[/htb]$ sudo ln -s /etc/nginx/sites-available/upload.conf /etc/nginx/sites-enabled/
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo systemctl restart nginx.service
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ tail -2 /var/log/nginx/error.log
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ ss -lnpt | grep 80
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ ps -ef | grep 2811
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo rm /etc/nginx/sites-enabled/default
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ curl -T /etc/passwd http://localhost:9001/SecretUploadDirectory/users.txt
```

##### Explanation about difference in previous file upload done in [[Linux File Transfer Methods#Linux - Upload Multiple Files]] and the above one is here [['curl -X POST vs curl -T' file uploads]]

  Catching Files over HTTP/S

```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo tail -1 /var/www/uploads/SecretUploadDirectory/users.txt 
