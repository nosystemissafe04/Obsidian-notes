...
```shell-session
0xxNosystemisSafe@htb[/htb]$ cat password.list
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ cat custom.rule
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ hashcat --force password.list -r custom.rule --stdout | sort -u > mut_password.list
0xxNosystemisSafe@htb[/htb]$ cat mut_password.list
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ ls /usr/share/hashcat/rules/
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ cewl https://www.inlanefreight.com -d 4 -m 6 --lowercase -w inlane.wordlist
0xxNosystemisSafe@htb[/htb]$ wc -l inlane.wordlist
