...
```bash
0xxNosystemisSafe@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --script rdp*
...
```bash
0xxNosystemisSafe@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p3389 --packet-trace --disable-arp-ping -n
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo cpan
...
```bash
0xxNosystemisSafe@htb[/htb]$ git clone https://github.com/CiscoCXSecurity/rdp-sec-check.git && cd rdp-sec-check
0xxNosystemisSafe@htb[/htb]$ ./rdp-sec-check.pl 10.129.201.248
...
```bash
0xxNosystemisSafe@htb[/htb]$ xfreerdp /u:cry0l1t3 /p:"P455w0rd!" /v:10.129.201.248
...
winrm is build upon http and it uses SOAP to establish connections the SOAP packets are encapsulated in https packets 

Another component that fits WinRM for administration is Windows Remote Shell (`WinRS`), which lets us execute arbitrary commands on the remote system. The program is even included on Windows 7 by default. Thus, with WinRM, it is possible to execute a remote command on another server. winrs is a program uses winrm .winrm is a microsoft implementation of WS-management protocol which is based on SOAP (simple object access protocol) . 

Services like remote sessions using PowerShell and event log merging require WinRM. It is enabled by default starting with the `Windows Server 2012` version, but it must first be configured for older server versions and clients, and the necessary firewall exceptions created.so on clients we have to configure winrm . follow these guides ==[[Credentials and Remoting#Powershell Remoting]]== this is beginner guide to understand and do remoting with powershell using winrm 
...
```bash
0xxNosystemisSafe@htb[/htb]$ nmap -sV -sC 10.129.201.248 -p5985,5986 --disable-arp-ping -n
...
![[Pasted image 20241020190639.png]]
in services we see http so by default it is using port 5985 http and we see upnp for service discovery . hence it is mainly used for managing devices and have ssdp as a component or a part of upnp which allows for service discovery or annoucing that there is a service so that another users can connect to it 
