...
unusual since SNMP typically uses **UDP** for communication. However, SNMP can be configured to use **TCP** in certain cases for more reliable communication, especially in environments where packet loss may be a concern.

 - SNMP also transmits **control commands** using ***agents*** over UDP port `161` 
 
	 **AGENTS** => An **SNMP agent** is software running on network devices (like routers, switches, servers, etc.). It collects information about the device's status and performance and reports it to the **SNMP manager** upon request.
	 
	 ***note*** => agents act as an **interface** between the SNMP manager and the devices being monitored or controlled
	 
	 The **Management Information Base (MIB)** is a database of the device’s properties, organized in a structured way using **Object Identifiers (OIDs)**
	 
	 ***note*** =>The agent exposes this MIB to the SNMP manager, allowing the manager to read (GET) or modify (SET) certain variables

- The client can set specific values in the device and change options and settings with these commands  
...
```bash
0xxNosystemisSafe@htb[/htb]$ cat /etc/snmp/snmpd.conf | grep -v "#" | sed -r '/^\s*$/d'
...
```bash
0xxNosystemisSafe@htb[/htb]$ snmpwalk -v2c -c public 10.129.14.128
...
- **Community string** is used as the form of authentication (e.g., `public` for read-only or `private` for read-write).
- SNMP walk will work as long as the correct community string is provided, and the SNMP agent allows access to the requested data. If the community string is incorrect or the agent does not permit the request, the SNMP walk will fail.

#### For **SNMPv3**:

- SNMPv3 uses **user-based authentication** and optional **encryption**.
- To perform an SNMP walk with SNMPv3, you must provide:
    - **Username**
    - **Authentication protocol (MD5 or SHA)** and password, if using `authNoPriv` or `authPriv`
    - **Encryption protocol (DES, AES)** and privacy password, if using `authPriv`
- SNMP walk will work if the correct credentials (username, password, encryption) are supplied, and the user has the required access privileges. Without correct authentication, the SNMP walk will fail.

So, **SNMP walk does not require authentication only in SNMPv1 or SNMPv2c**, where the community string is the "password." For SNMPv3, authentication is mandatory based on the security level being used (noAuthNoPriv, authNoPriv, or authPriv).
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo apt install onesixtyone
0xxNosystemisSafe@htb[/htb]$ onesixtyone -c /opt/useful/SecLists/Discovery/SNMP/snmp.txt 10.129.14.128
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo apt install braa
0xxNosystemisSafe@htb[/htb]$ braa <community string>@<IP>:.1.3.6.*   # Syntax
0xxNosystemisSafe@htb[/htb]$ braa public@10.129.14.128:.1.3.6.*
