...
![[Pasted image 20240928161745.png]]
...
```bash
0xxNosystemisSafe@htb[/htb]$ dig soa www.inlanefreight.com
...
u will get dns admin email and primary nameserver in soa record 
the domain admin 
...
```bash
0xxNosystemisSafe@htb[/htb]$ dig ns inlanefreight.htb @10.129.14.128
...
```bash
0xxNosystemisSafe@htb[/htb]$ dig CH TXT version.bind 10.129.120.85
...
```bash
0xxNosystemisSafe@htb[/htb]$ dig any inlanefreight.htb @10.129.14.128
...
### The Refresh Process OR When or how zone transfer carried out 

1. **Fetching the SOA Record**:
    
    - The slave DNS server is configured to query the master server for the SOA record at regular intervals defined by the **refresh time** (commonly set to one hour).
    - This means every hour, the slave server will reach out to the master server to get the current SOA record.
2. **Comparing Serial Numbers**:
    
    - Each time the slave fetches the SOA record, it checks the **serial number** of the SOA record from the master server.
    - The serial number is crucial because it indicates the version of the DNS zone data on the master server. A higher serial number means the data has been updated.
3. **Determining Data Consistency**:
    
    - If the **serial number** of the SOA record from the master is **greater** than the one stored on the slave:
        - This indicates that the data on the master server has changed since the last update to the slave.
        - In this case, the slave recognizes that its data is **out of date** and needs to be updated.
4. **Updating the Slave**:
    
    - If the slave determines that its serial number is outdated, it will initiate a ***zone transfer from the master server*** to fetch the updated records.
    - The process involves transferring all relevant DNS records from the master to the slave to ensure both servers are synchronized.

#### DIG - AXFR Zone Transfer

```bash
0xxNosystemisSafe@htb[/htb]$ dig axfr inlanefreight.htb @10.129.14.128
...
```bash
0xxNosystemisSafe@htb[/htb]$ dig axfr internal.inlanefreight.htb @10.129.14.128
...
```bash
	0xxNosystemisSafe@htb[/htb]$ for sub in $(cat /opt/useful/SecLists/Discovery/DNS/subdomains-top1million-110000.txt);do dig $sub.inlanefreight.htb @10.129.14.128 | grep -v ';\|SOA' | sed -r '/^\s*$/d' | grep $sub | tee -a subdomains.txt;done
...
```bash
0xxNosystemisSafe@htb[/htb]$ dnsenum --dnsserver 10.129.14.128 --enum -p 0 -s 0 -o subdomains.txt -f /opt/useful/SecLists/Discovery/DNS/subdomains-top1million-110000.txt inlanefreight.htb
...
***check on main domian*** 
***check on zone transfer on main domain***
***ckeck on subdomain . if not getting any subdomain try . crt.sh to find sub and brute force those subdomain .*** 
***check for zone transfer on subdomain*** 
***try bruteforcing 2ndlevelsubdomain*** 
