...
- BMCs are typically implemented as embedded ARM systems running Linux, and connected directly to the host's motherboard. BMCs are built into many motherboards but **can also be added to a system as a PCI card**. 
- **Most servers either come with a BMC or support adding a BMC**. 
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo nmap -sU --script ipmi-version -p 623 ilo.inlanfreight.local
...
#### [Hacktricks_IPMI blogs](https://book.hacktricks.xyz/network-services-pentesting/623-udp-ipmi)
