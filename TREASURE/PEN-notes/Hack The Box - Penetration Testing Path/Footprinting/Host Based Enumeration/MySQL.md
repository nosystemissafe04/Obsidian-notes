...
| ---- | ---- | ---- | ---- |
| customers | usernames | administrators | mederators q |
| email address | user information | permissions | passwords |
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo apt install mysql-server -y
0xxNosystemisSafe@htb[/htb]$ cat /etc/mysql/mysql.conf.d/mysqld.cnf | grep -v "#" | sed -r '/^\s*$/d'
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo nmap 10.129.14.128 -sV -sC -p3306 --script mysql*
...
```bash
0xxNosystemisSafe@htb[/htb]$ mysql -u root -h 10.129.14.132
...
```bash
0xxNosystemisSafe@htb[/htb]$ mysql -u root -pP4SSw0rd -h 10.129.14.128
