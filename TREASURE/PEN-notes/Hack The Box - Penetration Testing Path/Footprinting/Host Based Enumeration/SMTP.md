The `Simple Mail Transfer Protocol` (`SMTP`) is a protocol for sending emails in an IP network. It can be used between an email client and an outgoing mail server or between two SMTP servers. SMTP is often combined with the IMAP or POP3 protocols, which can fetch emails and send emails. In principle, it is a client-server-based protocol, although SMTP can be used between a client and a server and between two SMTP servers. In this case, a server effectively acts as a client.
 ...
 - newer SMTP servers also use other ports such as TCP port `587`.
...
 - The authentication data is protected and no longer visible in plaintext over the network
...
Both ports 587 and 465 are used for encrypted SMTP (Simple Mail Transfer Protocol) connections, but they serve different purposes and use different protocols:

- **Port 587 (Submission)**: This is the modern and recommended port for submitting email messages using **STARTTLS** encryption. STARTTLS upgrades an existing, plain-text connection to an encrypted one. Most modern email services use this port.
    
- **Port 465 (SMTPS)**: This was originally used for **SMTPS**, which is SMTP over implicit SSL/TLS. However, it was deprecated in favor of STARTTLS on port 587. Some legacy systems may still use port 465, but it's generally less common now.

##### Why Depricated 
- Port 465 was never formally registered with the IANA
- The primary method used with port 587 is **STARTTLS**, which starts an SMTP connection in plain text and upgrades to an encrypted connection. This makes it more flexible than implicit SSL/TLS on port 465, which requires encryption from the start. With STARTTLS, a server can support both encrypted and non-encrypted communications on the same port (587). which give us more flexibility 
---
---
...
- For this purpose, most modern SMTP servers support the **protocol extension** ***ESMTP***(Extended SMTP) with ***SMTP-Auth***(SMTP authentication)
- After sending his e-mail, the SMTP client, also known as `Mail User Agent` (`MUA`)=(client side email software), converts it into a header and a body and uploads both to the SMTP server.
-  This has a so-called `Mail Transfer Agent` (`MTA`)(server side email software), the software basis for sending and receiving e-mails.
- The MTA checks the **e-mail for size and spam** and then stores it. To relieve the MTA, it is occasionally preceded by a `Mail Submission Agent` (`MSA`), which checks the validity, i.e., the origin of the e-mail. This `MSA` is also called `Relay` server.
> 
> The MSA takes over these **initial responsibilities**, such as verifying the **validity of the email** (e.g., **ensuring the sender is authenticated and the email conforms to certain rules**). This **relieves** the **MTA** from having to do these tasks, allowing the MTA to focus more on **email routing and delivery**
...
2. 1. Users are not authenticated when a connection is established, and the sender of an email is therefore unreliable. As a result, open SMTP relays are often misused to send spam en masse. The originators use arbitrary fake sender addresses for this purpose to not be traced (mail spoofing). Today, many different security techniques are used to prevent the misuse of SMTP servers. For example, suspicious emails are rejected or moved to quarantine (spam folder). For example, responsible for this are the identification protocol [DomainKeys](http://dkim.org/) (`DKIM`), the [Sender Policy Framework](https://dmarcian.com/what-is-spf/) (`SPF`).
...
Each SMTP server can be configured in many ways, as can all other services. However, there are differences because the SMTP server is only responsible for sending and forwarding emails rather than tasks like receiving or storing them (which would typically be handled by other types of servers, such as IMAP or POP3 servers

#### Default Configuration

```bash
0xxNosystemisSafe@htb[/htb]$ cat /etc/postfix/main.cf | grep -v "#" | sed -r "/^\s*$/d"
...
```bash
0xxNosystemisSafe@htb[/htb]$ telnet 10.129.14.128 25
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo nmap 10.129.14.128 -sC -sV -p25
...
```bash
0xxNosystemisSafe@htb[/htb]$ sudo nmap 10.129.14.128 -p25 --script smtp-open-relay -v
...
![[Pasted image 20241002090323.png]]
user enumeration NSE script


**sometimes if the vrfy command or expn command are not supported we can use RCPT TO: <USER|EMAIL> to enumerate users on the smtp server if the user exist it will send ok response **
...
find all the mail servers of the org it is also possible that the single mail server is handling multiple domains 
use dig to enumerate more on domians 
finding 
...
very good enumeration on smtp 
### [Hacktricks - enum](https://book.hacktricks.xyz/network-services-pentesting/pentesting-smtp)
