...
Let's click _Accept Cookies_ to make sure nothing breaks. Once we do, we're presented with the full web page, which looks extremely convincing.
...
After clicking _Sign In_, the page grays out and presents a spinning wheel. Nothing else happens.
...
Let's inspect the output of our Python3 _http.server_ to figure out what's going on.
...
> Listing 8 - Finding which HTML page uses the **csrf_js** file
...
![Figure 6: Avoiding the OWASP CSRFGuard error alert](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/add199a1cd270e7ff13c30f15b79f377-phishing_basics_zoom03.png)
...
We'll eventually find that the form is enclosed in a _div_ element with an _id_ attribute named "app". This `<div>` element includes a header and two nested `<div>` tags that control the image display on the left side of the webpage and the login form placement on the right side. This is a good starting point, however, we're only seeing the outermost `<div>` tag in the `signin.html` file:
...
> Listing 11 - Finding only the _div_ with the "app" _id_
...
Let's copy as much as possible from the original page. We know that the _div_ has a header inside which we can most likely copy from within the developer tools by right-clicking on the _header_ element and selecting the _Copy_ followed by _OUTER HTML_.
...
Next, let's change the _id_ of the _div_ inside the **signin.html**, to avoid using the dynamically generated code, page and paste the output for the header.
...
> Listing 12 - Adding the _header_ to our new _div_
...
![Figure 12: Modifying the header with the help of ChatGPT](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/c84a80a1918878caad26a1a904116563-phishing_basics_clone04.png)

Figure 12: Modifying the header with the help of ChatGPT

Unfortunately, we're still missing the left-hand image and the login form. Let's find the image URL with the _Web Developer Tools_.

![Figure 13: Finding the image URL](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/c23c4c07abd4968ec682f32da90b5fe2-phishing_basics_clone05.png)
...
Let's insert this code after the _header_ tag inside our **signin.html** page.
...
![Figure 14: Updating our page to include the two div's generated by ChatGPT](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/60848d0d626bf1b793cd18fab2a71f27-phishing_basics_clone06.png)
...
![Figure 15: Updating our page to include the modifications requested from ChatGPT](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/bbd730d7187f098d019aed5646b76272-phishing_basics_clone07.png)
...
Next, we'll get ChatGPT's help with **custom_login.php**, which handles the POST requests and the email and password we're hoping to capture.
...
> Listing 19 - Requesting the code for the custom_login.php file
...
> Listing 20 - The code provided by ChatGPT for our custom_login.php file

Let's update the code in **custom_login.php**, create the **credentials.txt** file and apply appropriate permissions.
...
![Figure 16: Updating our page to include the modifications asked from ChatGPT](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/ce01c8a9a8a3d37ba6180d7b82f942f2-phishing_basics_clone08.png)

Figure 16: Updating our page to include the modifications asked from ChatGPT

After inputting the credentials, we'll click _Sign In_, at which point we are redirected to the actual Zoom website. Let's see if we captured the credentials.
...
1. What is the _id_ of the main div containing the main image and login form on the Zoom clone? (don't include anything before the '=')
...
![Figure 17: Add Exception for website](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/86310406f94e5931cc512289d04b7bf6-phishing_basics_send01a.png)

Figure 17: Add Exception for website

Here we click _Advanced_ and then _Accept the Risk and Continue_.

![Figure 18: Login page](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/5a5b6545291ffbc8a95c560e7d264af6-phishing_basics_send01b.png)
...
![Figure 19: Reply to sender and all recipients](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/1214d0d49ef0a4c6e820468ccc229758-phishing_basics_send01.png)
...
![Figure 20: Switch to HTML mode](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/e83175c93d49dfc533a9e5d42908a078-phishing_basics_html.png)

Figure 20: Switch to HTML mode

With the HTML editor open, we'll paste our malicious URL as a hyperlink.

![Figure 21: Adding a hyperlink to our malicious website](https://static.offsec.com/offsec-courses/PEN-200/imgs/phishing_basics/551e46cad587b2f9345b3b2da6744e7b-phishing_basics_send02.png)
