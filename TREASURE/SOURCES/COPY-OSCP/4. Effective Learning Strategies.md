...
Answer[View hints](https://portal.offsec.com/courses/pen-200-44065/learning/effective-learning-strategies-42562/practical-steps-42610/study-your-own-studies-42586#)
...
We'll consider the fact that the chmod command itself is just a file. It follows the same rules as other files on the system, including the same rules about permissions. It exists in a slightly different location (in the **/usr/bin/** directory) than our script, but the only reason we can run the _chmod +x find_employee_names.py_ command at all is because the **chmod** file has its permissions set to allow us to run it as a program.
...
Let's discuss where and how to follow along by focusing on the code presented in the Module. A keen learner may have noticed that all the code chunks use a similar style of formatting. Let's review one quickly:
...
![Figure 3: An Illustration of Cornell Notes](https://static.offsec.com/offsec-courses/LIBRARY/imgs//Effective_Learning_Strategies/619a2625dbb7c43ff2be15e86d4fd00e-cornell.png)
...
![Figure 4: Lab Machine completion contributing to success](https://static.offsec.com/offsec-courses/LIBRARY/imgs//Effective_Learning_Strategies/247356f94ff812b8abc45aec089e061e-pwk-stats.png)
