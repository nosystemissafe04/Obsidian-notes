...
Answer[View hints](https://portal.offsec.com/courses/pen-200-44065/learning/password-attacks-44959/wrapping-up-45020/wrapping-up-45024#)
...
3. Enumerate VM #2 (BRUTE2) and find another network service. Use the knowledge from this section to get access as the _itadmin_ user and find the flag.
...
2. The web page on VM #2 is password protected. Use Hydra to perform a password attack and get access as user _admin_. Once you have identified the correct password, enter it as the answer to this exercise.
...
[_Symmetric encryption_](https://en.wikipedia.org/wiki/Symmetric-key_algorithm) algorithms use the same key for both encryption and decryption. To send a message to another person, both sides need to know the key (password). If they exchange the key via an insecure channel, an attacker may intercept it. Additionally, the attacker may use a [_Man-in-the-middle_](https://en.wikipedia.org/wiki/Man-in-the-middle_attack) attack to gain access to the encrypted messages sent between the communication partners. If the attacker intercepts the key and also to the encrypted messages, he can decrypt and read them. This creates a huge security risk since the communication's security is based on the knowledge of a key, which needs to be known by both sides before starting communication. The [_Advanced Encryption Standard_](https://en.wikipedia.org/wiki/Advanced_Encryption_Standard) (AES) is an example of a symmetric encryption algorithm.
...
Before we begin cracking passwords, let's calculate the cracking time of various hash representations. The cracking time can be calculated by dividing the [_keyspace]_(https://www.hypr.com/security-encyclopedia/key-space) by the hash rate.
...
2. Access VM #2 via RDP as user _nadine_ with the password retrieved in the exercise of the section labeled "Password Manager" and leverage the methods from this section to extract Steve's NTLM hash. Use **best64.rule** for the cracking process and enter the plain text password as answer to this exercise.
...
Now that we've connected, LSASS should have cached the domain user's information in memory. Now let's log out by right-clicking on the Windows icon in the taskbar and choosing _Shut down or sign out_ -> _Sign out_.
